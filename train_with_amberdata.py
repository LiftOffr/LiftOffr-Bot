#!/usr/bin/env python3
"""
Train Models with Amberdata OHLCV Data

This script trains the improved hybrid model using OHLCV data from Amberdata API
at various timeframes (15m, 1h, 4h, 1d) and reports comprehensive performance
metrics including PnL, win rate, and Sharpe ratio.

Usage:
    python train_with_amberdata.py --pair BTC/USD --timeframe 1h
"""
...
# Import Amberdata client
try:
    from amberdata_client import AmberdataClient
except ImportError:
    print("amberdata_client.py not found. Please ensure it's in the same directory.")
    sys.exit(1)
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("amberdata_training.log")
    ]
)
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description="Train models with Amberdata OHLCV data")
    ...
    parser.add_argument("--api_key", type=str, default=None,
                        help="Amberdata API key (if not provided, will try to use AMBERDATA_API_KEY environment variable)")
...
def fetch_ohlcv_data(args) -> Optional[pd.DataFrame]:
    """Fetch OHLCV data from Amberdata API"""
    # Get API key
    ...
    logger.info(f"Fetching {args.timeframe} OHLCV data for {args.pair} from Amberdata API")
    
    # Create client
    client = AmberdataClient(api_key=api_key)
    
    # Fetch data
    df = client.get_historical_ohlcv(args.pair, timeframe=args.timeframe, days=args.days)
    
    if df.empty:
        logger.error(f"Failed to fetch data for {args.pair} ({args.timeframe})")
        return None
    ...
    filename = f"{HISTORICAL_DATA_DIR}/{pair_clean}_{args.timeframe}.csv"
...
def prepare_data_from_ohlcv(df: pd.DataFrame, args) -> Tuple:
    ...
    return X_train, y_train, X_val, y_val, X_test, y_test, scaler, feature_columns, df_labeled
...
def simulate_trading(
    ...
    if len(trading_signals) > 0:
        ...
        days = (trading_signals['timestamp'].iloc[-1] - trading_signals['timestamp'].iloc[0]) / (24 * 60 * 60)
    ...
    pair_clean = df.iloc[0]['close'] if 'pair' not in df.columns else df.iloc[0]['pair'].replace('/', '_').lower()
...
def main():
    ...
    # Print banner
    logger.info("\n" + "=" * 80)
    logger.info("TRAIN MODEL WITH AMBERDATA OHLCV DATA")
    logger.info("=" * 80)
    ...
    # Fetch OHLCV data
    df = fetch_ohlcv_data(args)
    if df is None or df.empty:
        logger.error("Failed to fetch OHLCV data. Exiting.")
        return 1
    
    # Prepare data
    X_train, y_train, X_val, y_val, X_test, y_test, scaler, feature_columns, df_labeled = prepare_data_from_ohlcv(df, args)
    ...
    # Train model
    model, history, model_path, training_time = train_model(model, X_train, y_train, X_val, y_val, args)
    
    # Evaluate model
    metrics = evaluate_model(model, X_test, y_test, df_labeled, scaler, feature_columns, args.sequence_length)
    
    # Save training report
    save_training_report(args.pair, args.timeframe, metrics, training_time, model_path)
    
    # Update ML config
    if args.update_ml_config:
        update_ml_config(args.pair, args.timeframe, model_path, metrics, args.max_portfolio_risk)
